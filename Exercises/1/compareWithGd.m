function compareWithGd(algo)

%generation of examples and targets
x=0:0.2:3*pi; 

y=sin(x); %label = {'sin(x)'};
y = awgn(y,10,'measured'); label = {'sin(x) + Noise'};

%y=sin(x.^2); label = {'y = sin(x.^2)'};
%y=log(x).*sin(x).*sin(x.^2);
...
for k=1:length(hiddenUnits)
    ...
    net1=feedforwardnet(hiddenUnits(k), 'traingd');
    ...
    for i=1:length(EpochsArr)
        ...
        title(title_str);
        legend('target', 'traingd', algo, 4);
        ylabel(label{1});
        xlabel('x');
        title_str = strcat('Images/sine/', title_str);
        saveas(gcf,strcat(strip(title_str),'.png'));
        
        title_str = sprintf('With Noise, %d Epochs, %d Hidden units (%s)', EpochsArr(i), hiddenUnits(k), 'traingd');
