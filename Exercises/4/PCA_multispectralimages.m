...
n_factors=input('# factors to retain? (enter for default=2) \n');
...
name_file=input('file name? (enter for default=scene7.zip) \n','s');
...
disp('downloading and unzipping files from repository...this might take a while!');
...
disp('let us check how the first image of the  ensemble looks like.');
figure;
...
disp('compute PCA of multi-spectral image...');
...
%% save all you need for the (lossy) reconstruction of the multi-spectral image
disp('save all you need for the (lossy) reconstruction of the multi-spectral image...');
save  reflectances-compress Z proj_mat mean_ sizes;
save ffnames filename;
disp('done');

%% start from scratch and reconstruct the multi-spectral image from what you have just saved
clear all;
disp('let us now reconstruct the multi-spectral image from scratch...');
...
load reflectances;
disp('done. compute reconstruction error:');
NRMSE=sqrt(mean((reflectances(:)-X_(:)).^2))/(max(reflectances(:))-min(reflectances(:)));
disp(['normalized root mean square error = ',num2str(NRMSE)]);
s=dir('reflectances.mat');
sc=dir('reflectances-compress.mat');        
const=9.53674316e-7;
 disp([' ']);disp(['The size of the original reflectances file is: ',num2str(s.bytes*const),' MB. The size of the "compressed" file is: ',num2str(sc.bytes*const),' MB.']);    
